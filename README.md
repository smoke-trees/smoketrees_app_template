...
| `[--inject-data]` | Generate a data-injecting parser with `endpoint`, `items`, and `childTemplate` fields. | *off* |
...
dart run create_stac_parser.dart ProductTile collections --inject-data  # → widgets/collections/product_tile/ (with inject-data)
```
...
When a parser needs to fetch data from an API and inject it into a widget template, use the `--inject-data` flag. This generates a parser with built-in support for:

- **`endpoint`** — API endpoint to fetch data from
- **`items`** — Inline item data (used when `endpoint` is null)
- **`childTemplate`** — Widget template with `{{key}}` placeholders
- **`loadingWidget`**, **`errorWidget`**, **`emptyWidget`** — State widgets

```sh
dart run create_stac_parser.dart ProductCard collections --inject-data
```

This creates:

```text
lib/stac_runtime/widgets/collections/product_card/
├── st_product_card.dart
├── st_product_card_parser.dart
└── st_product_card.g.dart
```
...
```dart
@JsonSerializable(explicitToJson: true)
class ProductCard extends StacWidget {
  const ProductCard({
    this.endpoint,
    this.items,
...
The generated parser includes Dio integration and `injectData` from `lib/stac_runtime/utils/inject_data.dart`:

```dart
class ProductCardParser extends StacParser<ProductCard> {
  final Dio dio;
  ProductCardParser({Dio? dio}) : dio = dio ?? Dio();

  @override
  Widget parse(BuildContext context, ProductCard model) {
...
The DSL can then use it like:

```json
{
  "type": "st_product_card",
  ...
  "emptyWidget": { "type": "center", "child": { "type": "text", "data": "No product found" } }
}
```
