...
class UserController extends GetxController {
  ...
  Future<User?> fetchUserById(String id) async {
    try {
      ...
      if (response.status.error || (response.result?.isEmpty ?? true)) {
        ConsoleLogger.info('[UserController] fetchUserById: response.result is empty');
        return null;
      }
      ConsoleLogger.info('[UserController] fetchUserById: response.result is not empty');
  ...
  Future<ApiResponse?> signIn({
    ...
    try {
      ...
      if (!response.status.error) {
        ConsoleLogger.success("[UserController] Sign in successful ${response.result}");
        user = await AppUtils().extractUserDataFromJWT(
  ...
  Future<bool?> logout() async {
    try {
      ...
      if (response?.status.error ?? true) {
        ConsoleLogger.warn("[UserController] Logout failed: ${response?.message}");
        return false;
      }
      await clear();
      return true;
