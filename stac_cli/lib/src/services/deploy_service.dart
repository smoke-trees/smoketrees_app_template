...
class DeployService {
  ...
  /// Deploy all built JSON files from the lib Stac build directory.
  Future<void> deploy({String? projectPath}) async {
    ...
    if (await screensDir.exists()) {
      await for (final entity in screensDir.list()) {
        ...
        try {
          final rawString = await entity.readAsString();

          // Parse to validate + normalize, then re-encode
          final dynamic parsedJson = jsonDecode(rawString);
          final stacJson = jsonEncode(parsedJson);
          ...
          ConsoleLogger.error('✗ Invalid JSON in $fileName — $e');
          screenFail++;
        } catch (e) {
    ...
    if (await themesDir.exists()) {
      await for (final entity in themesDir.list()) {
        ...
        try {
          final rawString = await entity.readAsString();
          final dynamic parsedJson = jsonDecode(rawString);
          final themeJson = jsonEncode(parsedJson);
  ...
  String _resolveScreensApiUrl() {
    ...
    return '${env.baseApiUrl}/app-screens/deploy';
  }

  /// Resolve Cloud Function endpoint for themes.save
  String _resolveThemesApiUrl() {
    return '${env.baseApiUrl}/app-themes/deploy';
  }
}
