...
class BuildService {
  ...
  /// Returns every annotated artifact (screens + themes) declared in a
  /// file's [content]. Used by `stac watch` to discover entry files and to
  /// derive their real artifact names/types from the annotation arguments
  /// (e.g. `@StacScreen(screenName: "sign_in")` -> name "sign_in"), which
  /// is what the app queries the backend/dev-server with.
  ...
  /// Builds exactly one artifact (screen or theme) from [sourceFilePath],
  /// returning the raw JSON string identical to what a full `stac build`
  /// writes for that artifact. This is the single-target seam `stac watch`
  /// drives so an edit to one screen never rebuilds the whole project.
  ...
  Future<_StacFileAnalysis> _analyzeStacFile(
    ...
    try {
      ...
      // Prefer the project's fvm-pinned Dart so the language version the app
      // requires (pubspec environment.sdk) is honored.
...
''';
  ...
  Future<Map<String, dynamic>?> _executeFileInProject(
    ...
    try {
      ...
      // Prefer the project's fvm-pinned Dart so the wrapper can parse the
      // app's sources even when the app's pubspec requires a newer language
      // version than whatever `dart` is on PATH.
      final dart = FlutterSdk.resolveDartSync(projectDir) ?? 'dart';
      final result =
          await Process.run(
            dart,
