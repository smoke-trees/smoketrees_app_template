...
///
/// PLUG IN: `isEntryFile` and `buildTargetFor` are the two seams that need
/// your existing annotation/registration logic — whatever `stac_cli build`
/// already uses today to decide "this file is a screen/theme, here's its
/// name and type". Everything else (the graph walk, debouncing, coalescing)
/// is new and repo-agnostic.
class BuildTargetResolver {
  ...
  /// Given an entry file's path and content, return its BuildTarget (name
  /// resolved from the annotation, e.g. @StacScreen('home') -> name: 'home').
  final BuildTarget Function(String path, String content) buildTargetFor;
  ...
  // entry file path -> its raw content (so buildTargetFor can parse the
  ...
  Set<BuildTarget>? affectedTargets(Set<String> changedFiles) {
    ...
    return affectedEntries
        .map((p) => buildTargetFor(p, _entryContents[p]!))
        .toSet();
  }

  Set<BuildTarget> allTargets() =>
      _entryFiles.map((p) => buildTargetFor(p, _entryContents[p]!)).toSet();
